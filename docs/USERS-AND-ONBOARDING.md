...
### Deep primary dive — Indie Hackers / dev.to / Lobsters + X (verbatim builder voice)
The sharpest, most actionable layer — real quotes from builders *in our exact beachhead*:
- **The crowd is building helloo's category *right now*, dozens of times over** (Vist, Copana, AI Nexus, memduck, atombot…). **Reception pattern is the lesson:** *"local-first personal AI that knows me"* → **warmth**; *"autonomous agent that acts for you"* → **interrogation**; *"yet another cross-model memory"* → **silence** (some launched to 0 comments). **→ Lead with local-first + privacy + a concrete daily job, never "memory" as an abstraction.**
- **The forgetting pain is stated in near-identical words by strangers** — *"forgets everything… Every. Single. Time."* The clearest repeated wound. But a bare "we remember" pitch draws zero engagement — it must attach to a concrete workflow.
- **How they solve it today = plain markdown files** (`MEMORY.md`, `TASKS.md`, git-versioned); many explicitly reject vector DBs as overkill. The non-negotiable trio: **"human-readable, local, under version control."** → helloo's memory must be **exportable, human-readable, git-able plain files** — that's *the price of trust*, not a nice-to-have (reinforces Q38 + the inspect/edit/forget hub).
- **Reliability skepticism runs deep — they've been burned** (a silent timezone bug debugged for 3 months; agent "exit 0" reporting false test passes; 134 cold emails → 0 replies). *"The kill filter matters more than the workflow."* *"Informs > acts."* → routines must be **transparent, gated, auditable, kill-switchable from day one.** Strongest validation of the trust layer yet.
- **⚠️ Don't say "agent."** To this crowd "agent" = unreliable + bill-shock. Say **"automations that actually work — bounded, observable, cost-capped."** *(A real language change for the PRD/positioning.)*
- **Memory = correctness, not capacity.** The sophisticated end knows "remember everything" backfires: **poisoning** (a builder's assistant invented fake colleagues in 48h), **staleness** ("high-relevance facts become confidently wrong"), **sycophancy**. → pitch *trustworthy, correctable* memory (our supersession + inspect/forget), not more storage.
- **What actually pulls a switch:** (1) **reaches into my real environment** (files/shell/calendar/comms/browser — "Cowork envy"); (2) **provider-independence** ("independence, not loyalty" — a hedge against one vendor's price/behavior); (3) quality that *pushes back*; (4) keeps context across the switch. → position helloo as **"the layer that makes you independent of any one model and reaches into your real life,"** not "a better assistant."
- **Pricing is a raw nerve** — metered billing = real anxiety (*"silently drains $30 in 8 minutes"*); *"most AI pricing friction is not cost friction, it is **ambiguity friction**"*; **one-time/ownership pricing has real WTP** ($199–$249 for a self-hostable tool); a **2–3 month churn cliff** is structural; *"paying a subscription feels icky"*; VC-subsidized prices are expected to rise. → **flat, predictable, legible** pricing; ownership framing; a memory product that **accrues value the longer you use it** is the natural antidote to the churn cliff; **avoid metered memory pricing.**
- **Migration villain, ready-made:** ChatGPT's export *"does NOT include your project instructions… The system prompts you carefully wrote? Gone."* A concrete lock-in story to position against.

**"Own your AI" is now establishment rhetoric** (YC-stage: *"the intelligence you rent will never compound like the intelligence you own"*), so it's table stakes — differentiate on *how*, not the slogan. And **portability is already being shipped by others** (on-device + MCP-portable memory launches) → our edge is the **membrane (private/shared) + acts-across-accounts + trust layer**, not portability alone.

*(Primary = Hacker News + Indie Hackers + Lobsters, verbatim; dev.to article-level; X = search-snippet-level only, Nitter offline. **Still owed: real Reddit** — blocked here; needs Claude-in-Chrome connected in an interactive session, or pasted threads.)*
